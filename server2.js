...
async function main() {
    ...
    const filePath = 'Excel.xlsx'; // Your Excel file path
    ...
    for (let { rowNumber, keyword } of keywords) {
        ...
        // Write the longest and shortest suggestions into the respective columns
        sheet.getCell(`B${rowNumber}`).value = longest;
        sheet.getCell(`C${rowNumber}`).value = shortest;
    ...
    // Save the updated workbook with the same file name
    await workbook.xlsx.writeFile(filePath);
    console.log(`Excel file for ${dayName} updated successfully!`);
}
